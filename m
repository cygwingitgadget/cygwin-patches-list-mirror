Return-Path: <cygwin-patches-return-1908-listarch-cygwin-patches=sourceware.cygnus.com@cygwin.com>
Received: (qmail 9998 invoked by alias); 26 Feb 2002 12:38:00 -0000
...
diff -u -3 -p -a -u -p -b -B -r1.83 Makefile.in=0A=
--- Makefile.in	2002/02/10 21:26:27	1.83=0A=
+++ Makefile.in	2002/02/26 12:27:30=0A=
...
diff -u -3 -p -a -u -p -b -B -r1.78 dtable.cc=0A=
--- dtable.cc	2002/02/22 19:33:41	1.78=0A=
+++ dtable.cc	2002/02/26 12:27:32=0A=
...
+        break;=0A=
 	...
 	fh =3D NULL;=0A=
@@ -370,9 +379,9 @@ dtable::build_fhandler (int fd, DWORD de=0A=
=20=0A=
   if (unix_name)=0A=
     {=0A=
-      char new_win32_name[strlen (unix_name) + 1];=0A=
       if (!win32_name)=0A=
 	{=0A=
+          char new_win32_name[strlen (unix_name) + 1];=0A=
 	  char *p;=0A=
 	  /* FIXME: ? Should we call win32_device_name here?=0A=
 	     It seems like overkill, but... */=0A=
...
diff -u -3 -p -a -u -p -b -B -r1.112 fhandler.cc=0A=
--- fhandler.cc	2002/02/19 22:06:50	1.112=0A=
+++ fhandler.cc	2002/02/26 12:27:35=0A=
...
diff -u -3 -p -a -u -p -b -B -r1.108 fhandler.h=0A=
--- fhandler.h	2002/02/10 13:38:48	1.108=0A=
+++ fhandler.h	2002/02/26 12:27:38=0A=
@@ -70,8 +70,11 @@ enum=0A=
...
+=0A=
 ...
 class path_conv;=0A=
 class fhandler_disk_file;=0A=
@@ -1028,6 +1034,74 @@ class fhandler_dev_dsp : public fhandler=0A=
...
@@ -1043,7 +1117,10 @@ typedef union=0A=
...
diff -u -3 -p -a -u -p -b -B -r1.198 path.cc=0A=
--- path.cc	2002/02/22 19:33:41	1.198=0A=
+++ path.cc	2002/02/26 12:27:47=0A=
...
+            }=0A=
 	  /* devn should not be a device.  If it is, then stop parsing now. */=0A=
 	    ...
 	    {=0A=
@@ -1405,10 +1429,16 @@ mount_info::conv_to_win32_path (const ch=0A=
...
=20=0A=
   ...
   for (i =3D 0; i < nmounts; i++)=0A=
     {=0A=
       const char *path;=0A=
@@ -1472,7 +1502,7 @@ mount_info::conv_to_win32_path (const ch=0A=
...
=20=0A=
  out:=0A=
@@ -3233,7 +3263,8 @@ chdir (const char *in_dir)=0A=
...
+  if (!isvirtual_dev(devn))=0A=
     ...
     {=0A=
@@ -3253,8 +3284,9 @@ chdir (const char *in_dir)=0A=
...
diff -u -3 -p -a -u -p -b -B -r1.37 path.h=0A=
--- path.h	2002/01/14 20:39:59	1.37=0A=
+++ path.h	2002/02/26 12:27:48=0A=
...
--- /dev/null	Tue Feb 26 12:30:52 2002=0A=
+++ fhandler_proc.cc	Tue Feb 26 10:15:16 2002=0A=
@@ -0,0 +1,294 @@=0A=
...
+static const char *proc_listing[PROC_LINK_COUNT] =3D {=0A=
+  "registry",=0A=
+  "version",=0A=
+  "uptime"=0A=
+};=0A=
...
+ * fhandler_proc.=0A=
+ */=0A=
+static DWORD proc_fhandlers[PROC_LINK_COUNT] =3D {=0A=
+  FH_REGISTRY,=0A=
+  FH_PROC,=0A=
+  FH_PROC=0A=
+};=0A=
...
+  if (*path =3D=3D 0)=0A=
+    return FH_PROC;=0A=
+  for (int i =3D 0; i < PROC_LINK_COUNT; i++)=0A=
+    {=0A=
...
+  return res;=0A=
+}=0A=
--- /dev/null	Tue Feb 26 12:30:59 2002=0A=
+++ fhandler_process.cc	Tue Feb 26 10:01:16 2002=0A=
@@ -0,0 +1,289 @@=0A=
...
+=0A=
+static const char *process_listing[PROCESS_LINK_COUNT] =3D {=0A=
+  "ppid",=0A=
+  "exename",=0A=
+  "winpid",=0A=
+  "winexename",=0A=
...
+    return 2;=0A=
+=0A=
+  for (int i =3D 0; i < PROCESS_LINK_COUNT; i++)=0A=
...
+  return res;=0A=
+}=0A=
--- /dev/null	Tue Feb 26 12:31:02 2002=0A=
+++ fhandler_registry.cc	Tue Feb 26 11:11:53 2002=0A=
@@ -0,0 +1,505 @@=0A=
...
+ */=0A=
+const __off32_t REG_ENUM_VALUES_MASK =3D 0x8000000;=0A=
+=0A=
...
+ */=0A=
+static const char *registry_listing[ROOT_KEY_COUNT] =3D {=0A=
...
+    {=0A=
+      for (int i =3D 0; i < ROOT_KEY_COUNT; i++)=0A=
+        if (path_prefix_p=0A=
...
+  return hKey;=0A=
+}=0A=
--- /dev/null	Tue Feb 26 12:31:06 2002=0A=
...
2002-02-26  Christopher January <chris@atomice.net>
	...
	* dtable.cc (dtable::build_fhandler): Add entries for FH_PROC,
	...
	Set unix_name to the normalized posix path.
	...
	* path.cc (chdir): Replace check for FH_CYGDRIVE with more generic
	isvirtual_dev macro.
	Force setting of posix path for virtual fhandlers.
