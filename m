Return-Path: <cygwin-patches-return-1508-listarch-cygwin-patches=sourceware.cygnus.com@sources.redhat.com>
Received: (qmail 24262 invoked by alias); 21 Nov 2001 03:21:22 -0000
...
Received: (qmail 24233 invoked from network); 21 Nov 2001 03:21:21 -0000
Message-ID: <911C684A29ACD311921800508B7293BA037D2793@cnmail>
From: Mark Bradshaw <bradshaw@staff.crosswalk.com>
To: "'cygwin-patches@cygwin.com'" <cygwin-patches@cygwin.com>
Subject: [PATCH] mkpasswd.c - allows selection of specific user
Date: Mon, 15 Oct 2001 01:12:00 -0000
MIME-Version: 1.0
X-Mailer: Internet Mail Service (5.5.2653.19)
Content-Type: multipart/mixed;
	boundary="----_=_NextPart_000_01C17238.F1F64C30"
X-SW-Source: 2001-q4/txt/msg00040.txt.bz2

This message is in MIME format. Since your mail reader does not understand
this format, some or all of this message may not be legible.

------_=_NextPart_000_01C17238.F1F64C30
...
Content-Disposition: attachment;
	filename="mkpasswd.diff.cvs.txt"
Content-length: 4749
...
=20
 ...
 SID_IDENTIFIER_AUTHORITY sid_nt_auth =3D {SECURITY_NT_AUTHORITY};
@@ -27,6 +28,7 @@ NET_API_STATUS WINAPI (*netapibufferfree
 NET_API_STATUS WINAPI (*netuserenum)(LPWSTR,DWORD,DWORD,PBYTE*,DWORD,PDWOR=
D,PDWORD,PDWORD);
 NET_API_STATUS WINAPI (*netlocalgroupenum)(LPWSTR,DWORD,PBYTE*,DWORD,PDWOR=
D,PDWORD,PDWORD);
 NET_API_STATUS WINAPI (*netgetdcname)(LPWSTR,LPWSTR,PBYTE*);
+NET_API_STATUS WINAPI (*netusergetinfo)(LPWSTR,LPWSTR,DWORD,PBYTE*);
=20
...
@@ -48,6 +50,8 @@ load_netapi ()
     return FALSE;
   if (!(netgetdcname =3D (void *) GetProcAddress (h, "NetGetDCName")))
     return FALSE;
+  if (!(netusergetinfo =3D (void *) GetProcAddress (h, "NetUserGetInfo")))
+    return FALSE;
=20
   return TRUE;
 }
...
+	    const char * passed_home_path, int id_offset, char *disp_username)
 {
   USER_INFO_3 *buffer;
   DWORD entriesread =3D 0;
@@ -112,6 +116,7 @@ enum_users (LPWSTR servername, int print
   DWORD resume_handle =3D 0;
   DWORD rc;
   char ansi_srvname[256];
+  WCHAR uni_name[512];
=20
...
@@ -120,6 +125,12 @@ enum_users (LPWSTR servername, int print
     {
       DWORD i;
=20
+    if (disp_username !=3D NULL) {
+      MultiByteToWideChar (CP_ACP, 0, disp_username, -1, uni_name, 512 );
+      rc =3D netusergetinfo(servername, (LPWSTR) & uni_name, 3, (LPBYTE *)=
 &buffer );
+      entriesread=3D1;
+    }
+    else=20
       rc =3D netuserenum (servername, 3, FILTER_NORMAL_ACCOUNT,
 			(LPBYTE *) & buffer, 1024,
 			&entriesread, &totalentries, &resume_handle);
@@ -134,7 +145,9 @@ enum_users (LPWSTR servername, int print
 	  break;
...
-	  fprintf (stderr, "NetUserEnum() failed with %ld\n", rc);
+	  fprintf (stderr, "NetUserEnum() failed with error %ld.\n", rc);
+	  if ( rc =3D=3D NERR_UserNotFound )=20
+	    fprintf (stderr, "That user doesn't exist.\n");
 	  exit (1);
 	}
=20
...
+  fprintf (stderr, "   -u,--username username  only return information for=
 the specified user\n");
   ...
   fprintf (stderr, "One of `-l', `-d' or `-g' must be given on NT/W2K.\n");
   return 1;
@@ -394,11 +408,12 @@ struct option longopts[] =3D {
   {"no-mount", no_argument, NULL, 'm'},
   {"no-sids", no_argument, NULL, 's'},
   {"path-to-home",required_argument, NULL, 'p'},
+  {"username",required_argument, NULL, 'u'},
...
=20
-char opts[] =3D "ldo:gsmhp:";
+char opts[] =3D "ldo:gsmhpu:";
=20
 int
 main (int argc, char **argv)
@@ -414,6 +429,7 @@ main (int argc, char **argv)
...
=20
   if (print_local)
-    enum_users (NULL, print_sids, print_cygpath, passed_home_path, 0);
+    enum_users (NULL, print_sids, print_cygpath, passed_home_path, 0, disp=
_username);
=20
   if (servername)
     netapibufferfree (servername);
